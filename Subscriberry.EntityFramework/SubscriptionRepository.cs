...
namespace Subscriberry.EntityFramework
{
	public class SubscriptionRepository : ISubscriptionRepository
	{
		...
		public SubscriptionRepository(string connectionString)
		{
			...
			_dbContext = new SubscriberryDbContext(optionBuilder.Options);
		}

		public SubscriptionRepository(DataContext dataContext)
		{
			this._dbContext = dataContext.DbContext;
		}
		...
		{
			var data = _dbContext.Groups.FirstOrDefault(g => g.Name.Equals(name));
			if (data != null)
				return data;

			var newGroup = _dbContext.Groups.Add(new SubscriptionGroup
			{
				Name = name
			});
			_dbContext.SaveChanges();
		...
		public void EnsureSubscriptionData(int subscriptionId, string subscriptionEvent, string group)
		{
			...
			_dbContext.Subscriptions.Add(new Subscription(subscriptionId, subscriptionEvent, data.Id));
			_dbContext.SaveChanges();
		}
